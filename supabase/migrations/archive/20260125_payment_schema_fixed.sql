-- ==========================================
-- CARVIS - PAYMENT & COMMISSION SCHEMA (FIXED)
-- PayTR entegrasyonu için gerekli tablolar
-- ==========================================
...
-- 7. RLS POLİTİKALARI (Basitleştirilmiş)
...
-- Commissions politikaları
DROP POLICY IF EXISTS "Sellers can view their commissions" ON public.commissions;
CREATE POLICY "Sellers can view their commissions" ON public.commissions 
  FOR SELECT USING (auth.uid() = seller_id);

-- Seller balances politikaları
DROP POLICY IF EXISTS "Sellers can view their balance" ON public.seller_balances;
CREATE POLICY "Sellers can view their balance" ON public.seller_balances 
  FOR SELECT USING (auth.uid() = seller_id);

DROP POLICY IF EXISTS "System can manage balances" ON public.seller_balances;
CREATE POLICY "System can manage balances" ON public.seller_balances 
  FOR ALL USING (true);

-- Withdrawal requests politikaları
DROP POLICY IF EXISTS "Sellers can manage their withdrawals" ON public.withdrawal_requests;
CREATE POLICY "Sellers can manage their withdrawals" ON public.withdrawal_requests 
...
-- 9. TRİGGER: Sipariş oluşturulduğunda komisyon kaydı oluştur
CREATE OR REPLACE FUNCTION create_commission_on_order()
RETURNS TRIGGER AS $$
BEGIN
  -- Eğer sipariş ödendiyse komisyon oluştur
