-- ==========================================
-- CARVIS - PAYMENT & COMMISSION SCHEMA
-- PayTR entegrasyonu için gerekli tablolar
-- ==========================================

-- 1. ORDERS TABLOSU (Zaten var, ama kontrol edelim)
-- Eğer yoksa oluştur
...
-- 2. TRANSACTIONS TABLOSU (PayTR işlemleri)
...
-- 3. COMMISSIONS TABLOSU (Komisyon ödemeleri)
...
-- 4. SELLER_BALANCES TABLOSU (Satıcı bakiyeleri)
...
-- 5. WITHDRAWAL_REQUESTS TABLOSU (Para çekme talepleri)
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'transactions' AND policyname = 'Users can view their transactions') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'seller_balances' AND policyname = 'Sellers can view their balance') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'withdrawal_requests' AND policyname = 'Sellers can manage their withdrawals') THEN
...
-- 8. TRİGGER: Sipariş oluşturulduğunda komisyon kaydı oluştur
