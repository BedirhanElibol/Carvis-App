  ...
  try {
    ...
    const fetchPromises = CITIES.map(async (city) => {
      try {
        ...
        // Opet API'si LPG'yi her zaman vermiyor, bu yüzden bir katsayı ile tahmini oran hesaplanabilir 
        // ya da mevcutsa direkt alınabilir.
        let lpgObj = targetDistrict.prices.find((p: any) => p.productShortName === "LPG" || p.productName.includes("LPG") || p.productName.includes("Otogaz"));
    ...
    if (allPrices.length > 0) {
      const { error } = await supabaseClient
        ...
        .upsert(allPrices, { onConflict: 'province_code' })

      if (error) throw error
    }

    return new Response(JSON.stringify({ success: true, count: allPrices.length }), {
