      ...
      <div className="sticky top-0 z-20 bg-slate-950/40 backdrop-blur-2xl border-b border-white/5 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <Icons.ArrowLeft size={20} className="text-slate-300" />
            </button>
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 flex items-center justify-center border border-white/10 shadow-lg font-black text-primary-500">
                {otherUser?.full_name?.[0]?.toUpperCase() ||
                  otherUser?.company_name?.[0]?.toUpperCase() ||
                  "?"}
              </div>
              <div>
                <h1 className="text-sm font-black text-white truncate w-32 tracking-tighter uppercase font-sans">
              ...
              <div key={message.id} className="space-y-4">
                {showDate && (
                  <div className="flex items-center justify-center my-6">
                    <div className="bg-slate-900/50 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-widest border border-white/5 font-sans">
        ...
        <form onSubmit={handleSend} className="flex items-center gap-3">
          <div className="flex-1 relative group">
            <input
              ...
              className="w-full bg-slate-900/80 border border-white/10 rounded-[2rem] px-5 py-4 text-sm font-medium focus:border-primary-500/50 focus:outline-none transition-all pr-12 placeholder-slate-600 font-sans"
              disabled={sending}
            />
          </div>
          <button
          ...
          >
            {sending ? (
              ...
              <Icons.Send size={22} className="text-white -mr-1" />
            )}
          </button>
        </form>
      </div>
