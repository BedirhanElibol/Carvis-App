  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="glass-card w-full max-w-2xl max-h-[80vh] rounded-[2.5rem] overflow-hidden flex flex-col relative animate-slide-up border border-black/10 dark:border-white/10">
        ...
        {/* Content */}
        <div className="p-8 overflow-y-auto custom-scrollbar text-slate-600 dark:text-slate-300 space-y-6 font-sans text-sm leading-relaxed">
          {doc.content.split('\n\n').map((paragraph, idx) => (
            <p key={idx}>{paragraph}</p>
          ))}
          
          <div className="p-4 bg-white dark:bg-slate-900/50 rounded-2xl border border-black/5 dark:border-white/5 text-[10px] text-slate-500 italic">
            Son Güncelleme: {new Date().toLocaleDateString('tr-TR')}
          </div>
        </div>

        {/* Footer */}
        <div className="p-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex justify-end">
          <button
            onClick={onClose}
            className="bg-primary-600 hover:bg-primary-500 text-slate-900 dark:text-white px-8 py-3 rounded-xl font-bold transition-all active-scale"
          >
            Anladım
          </button>
        </div>
