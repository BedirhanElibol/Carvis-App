...
const ProductDetailScreen = () => {
    ...
    return (
        <div className="p-5 pb-32 space-y-6 animate-fade-in relative min-h-screen bg-slate-950 text-white">
            <div className="absolute top-[-5%] left-[-5%] w-[40%] h-[40%] bg-primary-600/10 rounded-full blur-[80px] pointer-events-none"></div>

            <div className="flex justify-between items-center mb-4">
                <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-white font-black text-[10px] uppercase tracking-widest glass-card px-5 py-3 rounded-2xl shadow-2xl hover:bg-white/10 active-scale border border-white/10">
                    <ChevronLeft size={16} className="text-primary-500" /> {t.back}
                </button>
                <button onClick={() => toggleFavorite(product.id)} className="p-3 glass-card rounded-full shadow-2xl text-slate-500 hover:text-red-500 transition-all border border-white/10 active-scale">
                    <Heart size={20} className={favorites.includes(product.id) ? "fill-red-500 text-red-500" : ""} />
                </button>
            </div>

            <div className="glass-card !p-6 space-y-6 shadow-2xl rounded-[3rem] border border-white/5 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-48 h-48 bg-primary-600/10 rounded-full blur-3xl -mr-20 -mt-20"></div>

                <div className="w-full h-80 bg-slate-900 rounded-[2rem] overflow-hidden shadow-inner relative border border-white/5">
                    ...
                    {product.certified && (
                        <div className="absolute top-5 left-5 bg-primary-600 text-white px-4 py-2 rounded-full text-[10px] font-black flex items-center gap-2 shadow-2xl border border-primary-500/30 uppercase tracking-widest animate-pulse">
                ...
                <div className="space-y-4 relative z-10">
                    <div>
                        ...
                        <h3 className="font-black text-3xl text-white italic tracking-tighter leading-none mb-3">{product.name}</h3>
                        ...
                        <div className="glass-card border border-primary-500/20 p-5 rounded-[2rem] cursor-pointer hover:bg-primary-500/5 transition-all shadow-xl active-scale" onClick={() => setShowCert(!showCert)}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-black text-white text-sm italic tracking-tight">RAPIDSY DİJİTAL SERTİFİKA</h4>
                            ...
                            {showCert && (
                                <div className="mt-5 pt-5 border-t border-white/10 space-y-3 animate-slide-up">
                                    ...
                                    <div className="flex justify-between items-center text-[10px] font-black tracking-widest uppercase">
                                        ...
                                        <span className="text-white">RPD-988231-X</span>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="pt-8 border-t border-white/5 mt-4">
                        <div className="flex justify-between items-end mb-6">
                            <div>
                                ...
                                <p className="font-black text-5xl text-white italic tracking-tighter">{buyBoxPrice.toLocaleString()} <span className="text-2xl not-italic ml-1">₺</span></p>
                            </div>
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => addToCart(product, winnerOffer)} className="flex-1 glass-card border border-white/10 text-white py-4.5 rounded-[1.8rem] font-black text-xs uppercase tracking-widest hover:bg-white/5 transition-all active-scale shadow-2xl">
                                {t.addToCart}
                            </button>
                            <button onClick={handleBuyNow} className="flex-[1.5] bg-primary-600 text-white py-4.5 rounded-[1.8rem] font-black text-xs uppercase tracking-widest hover:bg-primary-500 transition-all shadow-2xl shadow-primary-900/50 flex justify-center items-center gap-3 active-scale">
