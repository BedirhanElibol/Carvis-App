    ...
    return (
        <div className="p-5 pb-32 space-y-4 animate-fade-in">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-black text-3xl text-white italic tracking-tighter">{t.parts}</h3>
                <div onClick={() => navigate('/checkout')} className="relative p-3 glass-card rounded-2xl cursor-pointer hover:bg-white/10 transition-all active-scale shadow-2xl border border-white/10">
                    <ShoppingBag size={24} className="text-primary-400" />
                    {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-accent-600 text-white w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-black ring-2 ring-slate-900 animate-pulse">{cartCount}</span>}
                </div>
            </div>
            ...
            <div className="flex gap-2 mb-6 sticky top-0 z-20 py-2">
                <div className="flex-1 glass-card border border-white/10 rounded-2xl flex items-center px-4 py-3.5 shadow-2xl focus-within:border-primary-500/50 transition-all backdrop-blur-xl">
                    <Search size={20} className="text-slate-500 mr-3" />
                    <input type="text" placeholder="Parça, marka veya kategori ara..." className="bg-transparent w-full outline-none text-sm text-white placeholder-slate-500 font-medium" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
                <button onClick={() => setShowFilterModal(true)} className={`p-4 rounded-2xl border transition-all active-scale shadow-2xl ${filterCriteria.min || filterCriteria.max || filterCriteria.stock ? 'bg-primary-600 border-primary-500 text-white' : 'glass-card border-white/10 text-slate-400 hover:text-white'}`}>
                    <Filter size={22} />
                </button>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                {PART_CATEGORIES.map((cat, i) => (
                    <button key={i} className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold border transition shadow-md ${i === 0 ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'}`}>
                        {cat}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                {filteredProducts.map(product => {
                    ...
                    return (
                        <div key={product.id} onClick={() => setSelectedProduct(product)} className="glass-card p-3 rounded-[2rem] border border-white/5 shadow-2xl hover:border-primary-500/30 transition-all cursor-pointer group relative active-scale overflow-hidden">
                            <div className="absolute inset-0 bg-gradient-to-br from-primary-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                            <div className="absolute top-3 left-3 z-20 flex flex-col gap-1">
                                {product.certified && (
                                    <div className="bg-primary-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 shadow-lg border border-primary-400/20 uppercase tracking-widest">
                                        <ShieldCheck size={8} /> ONAYLI
                                    </div>
                                )}
                                {currentVehicle && (
                                    <div className="bg-green-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full flex items-center gap-1 shadow-lg border border-green-400/20 uppercase tracking-widest">
                                        <CircleCheck size={8} /> %100 UYUMLU
                                    </div>
                                )}
                            </div>

                            <button onClick={(e) => { e.stopPropagation(); toggleFavorite(product.id); }} className="absolute top-3 right-3 p-2 glass-card rounded-full text-slate-500 hover:text-red-500 transition-all z-10 shadow-lg border border-white/5 active-scale">
                                <Heart size={14} className={favorites.includes(product.id) ? "fill-red-500 text-red-500" : ""} />
                            </button>
                            <div className="w-full h-36 bg-slate-900/50 rounded-2xl mb-4 overflow-hidden border border-white/5">
                            ...
                            <div className="space-y-1 relative z-10 px-1">
                                ...
                                <h4 className="font-bold text-xs text-slate-100 leading-tight line-clamp-2 h-8">{product.name}</h4>
                                <div className="flex justify-between items-end mt-3 border-t border-white/5 pt-2">
                                    <div>
                                        ...
                                        <p className="font-black text-base text-white">{(winnerOffer?.price || 0).toLocaleString()} ₺</p>
                                    </div>
                                    <div className="flex gap-1">
                                        {/* Only Add to Cart Button */}
                                        <button onClick={(e) => { e.stopPropagation(); addToCart(product, winnerOffer); }} className="w-full bg-slate-800 text-white p-2.5 rounded-xl hover:bg-slate-700 transition-all shadow-lg active-scale flex items-center justify-center gap-2">
                                            <CirclePlus size={18} />
