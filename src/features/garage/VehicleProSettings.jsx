...
const VehicleProSettings = ({ isOpen, onClose, vehicle }) => {
  ...
  const getExpenseIcon = (type) => {
    switch (type) {
      ...
      default: return <Icons.CreditCard className="text-slate-500 dark:text-slate-400" />;
    }
  };
              ...
              <div className="space-y-6">
                ...
                {showExpenseForm && (
                  <form onSubmit={handleAddExpenseSubmit} className="p-6 bg-white dark:bg-slate-900/60 rounded-3xl border border-black/10 dark:border-white/10 space-y-4 animate-in slide-in-from-top-4 duration-300">
                    ...
                    <button 
                      type="submit"
                      className="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-slate-900 dark:text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all"
                    >
                      KAYDET
                    </button>
                  </form>
