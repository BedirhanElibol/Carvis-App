...
import { Wrench, Package, Send, TriangleAlert } from 'lucide-react';

const VehicleDemandForm = ({ vehicle, onSubmit, initialDemandType = 'part', initialDescription = '' }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 animate-slide-up">
      ...
      <div className="bg-blue-600 p-4 rounded-2xl mb-6 text-white shadow-lg shadow-blue-200">
        <p className="text-[10px] font-black uppercase tracking-widest opacity-80">Seçili Araç</p>
      ...
      <h2 className="text-xl font-black text-slate-900 mb-4 text-center uppercase tracking-tight">Ne Lazım?</h2>

      {/* Talep Türü Seçimi */}
      <div className="grid grid-cols-2 gap-3 mb-6">
        <button
          onClick={() => setDemandType('part')}
          className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center space-y-2 ${demandType === 'part' ? 'border-blue-600 bg-blue-50' : 'border-slate-100 bg-slate-50'
            }`}
        >
          <Package className={demandType === 'part' ? 'text-blue-600' : 'text-slate-400'} />
          <span className="text-xs font-black uppercase">Yedek Parça</span>
        </button>
        <button
          onClick={() => setDemandType('service')}
          className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center space-y-2 ${demandType === 'service' ? 'border-blue-600 bg-blue-50' : 'border-slate-100 bg-slate-50'
            }`}
        >
          <Wrench className={demandType === 'service' ? 'text-blue-600' : 'text-slate-400'} />
      ...
      {/* Detay Açıklama */}
      <div className="mb-6">
        <label className="text-[10px] font-black text-slate-400 uppercase ml-2 mb-1 block">Talebinizi Açıklayın</label>
        <textarea
          rows="4"
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder={demandType === 'part' ? "Örn: Ön fren balatası, Yağ filtresi..." : "Örn: Motor arıza lambası yanıyor, Periyodik bakım..."}
