...
const LocationRequiredScreen = ({ onRetry }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-slate-950 flex flex-col items-center justify-center p-8 text-center overflow-hidden">
      {/* Background Effects */}
      ...
      <div className="relative z-10 max-w-sm flex flex-col items-center animate-in fade-in zoom-in-95 duration-700">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute -top-12 -left-4 flex items-center gap-2 text-slate-500 hover:text-white transition-colors group"
        >
          <Icons.ChevronLeft
            size={20}
        ...
        <div className="relative mb-10">
          <div className="w-24 h-24 bg-gradient-to-tr from-primary-600 to-primary-600 rounded-[2.5rem] flex items-center justify-center shadow-[0_0_50px_rgba(37,99,235,0.4)] rotate-12 group-hover:rotate-0 transition-transform duration-500">
            <Icons.MapPin size={48} className="text-white -rotate-12" />
          </div>
          <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-slate-900 border-2 border-slate-800 rounded-full flex items-center justify-center shadow-xl animate-bounce">
            <Icons.ShieldAlert size={20} className="text-red-500" />
          </div>
        </div>

        <h1 className="text-4xl font-black text-white tracking-tighter uppercase mb-4 leading-none">
        ...
        <div className="w-full space-y-4">
          <button
            onClick={onRetry}
            className="w-full bg-primary-600 hover:bg-primary-500 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-2xl shadow-primary-900/50 transition-all active-scale-95 group"
          >
            <Icons.RefreshCw
              size={20}
          ...
          <button
            onClick={() => navigate(-1)}
            className="w-full bg-slate-900 border border-white/5 hover:bg-slate-800 text-slate-400 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all active-scale-95"
          >
