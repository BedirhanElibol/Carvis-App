            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                        <div className={`${config.bg} ${config.color} px-2 py-1 rounded-lg flex items-center gap-1.5 text-xs font-bold`}>
                            <StatusIcon size={14} />
                            {config.label}
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <h3 className="text-lg font-bold text-white">
                            {quote.seller?.company_name || quote.seller?.full_name || 'Satıcı'}
                        </h3>
                        {quote.seller?.is_verified && (
                            <div className="flex items-center gap-1 bg-primary-500/10 text-primary-400 px-1.5 py-0.5 rounded-md border border-primary-500/20" title="Doğrulanmış Satıcı">
                                <CheckCircle size={10} className="fill-current" />
                                <span className="text-[9px] font-black uppercase tracking-tighter">Resmi</span>
                            </div>
                        )}
                    </div>
                    {quote.seller?.seller_rating > 0 && (
                        <div className="flex items-center gap-1 mt-1">
                            ...
                            <span className="text-xs text-slate-400">{quote.seller.seller_rating.toFixed(1)}</span>
                        </div>
                    )}
                </div>
