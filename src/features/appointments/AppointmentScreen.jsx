...
const AppointmentScreen = () => {
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            ...
            case 'confirmed':
                return { icon: CheckCircle, color: 'text-green-400', bg: 'bg-green-500/10', label: 'Onaylandı' };
            case 'completed':
                return { icon: CheckCircle, color: 'text-blue-400', bg: 'bg-blue-500/10', label: 'Tamamlandı' };
            case 'cancelled':
                return { icon: XCircle, color: 'text-red-400', bg: 'bg-red-500/10', label: 'İptal Edildi' };
            default:
                return { icon: AlertCircle, color: 'text-slate-400', bg: 'bg-slate-500/10', label: 'Bilinmiyor' };
        }
    };
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">{filteredAppointments.length} randevu</p>
                        </div>
                    </div>
                ...
                <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-hide">
                    {[
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${filter === option.value
                            ...
                            >
                                ...
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex-1">
                                        <div className={`flex items-center gap-1.5 mb-2 ${config.bg} ${config.color} px-2 py-1 rounded-lg w-fit text-xs font-bold`}>
                                            <StatusIcon size={14} />
                                            {config.label}
                                        </div>
                                        ...
                                        <p className="text-sm text-slate-400">
                                            {isSeller ? appointment.customer?.full_name : appointment.seller?.company_name || appointment.seller?.full_name}
                                        </p>
                                    </div>
                                </div>
