...
import { Wrench, Plus, Trash2, Edit2, ShieldCheck, Clock, Save, X } from 'lucide-react';
...
const MechanicServices = () => {
    ...
    const [services, setServices] = useState([
        { id: 1, name: 'Periyodik Bakım (Benzinli)', price: 1500, duration: '90 dk', includes: ['Motor Yağı', 'Yağ Filtresi', 'Hava Filtresi', 'Genel Kontrol'] },
        { id: 2, name: 'Fren Balatası Değişimi (Ön)', price: 800, duration: '45 dk', includes: ['İşçilik', 'Balata Spreyi'] },
        { id: 3, name: 'Kışlık Bakım Paketi', price: 600, duration: '30 dk', includes: ['Antifriz', 'Lastik Kontrolü', 'Akü Ölçümü'] },
    ]);

    const [newService, setNewService] = useState({ name: '', price: '', duration: '', includes: '' });
    ...
    return (
        <div className="space-y-6">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black italic tracking-tighter text-white">Servis Hizmetleri</h1>
                    <p className="text-slate-400 text-sm">Müşterilerinize sunduğunuz paketleri düzenleyin</p>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {services.map(service => (
                    <div key={service.id} className="glass-card p-6 rounded-2xl border border-white/5 bg-slate-900/50 hover:border-primary-500/30 transition-all group relative">
                        <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none group-hover:opacity-10 transition-opacity">
                            <Wrench size={80} />
                        </div>
                        
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="font-bold text-white text-lg pr-4">{service.name}</h3>
                        ...
                        <div className="flex items-center gap-2 text-slate-400 text-xs font-bold mb-4">
                            ...
                            <ShieldCheck size={14} /> Garantili İşçilik
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
                    <div className="bg-slate-900 w-full max-w-lg rounded-3xl border border-white/10 shadow-2xl overflow-hidden animate-in zoom-in-95">
                        ...
                        <form onSubmit={handleAddService} className="p-6 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1 ml-1">Hizmet Adı</label>
                                <input 
                                    required
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 mb-1 ml-1">Süre</label>
                                    <input 
                                        required
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-400 mb-1 ml-1">Kapsam (Virgülle ayırın)</label>
                                <textarea 
                                    required
                                    value={newService.includes}
                                    onChange={e => setNewService({...newService, includes: e.target.value})}
                                    className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:border-primary-500 outline-none transition-colors h-24 resize-none"
