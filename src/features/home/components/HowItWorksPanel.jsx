...
const HowItWorksPanel = memo(({ t }) => {
  return (
    <div className="bg-white dark:bg-[#0a0f24]/60 border border-black/5 dark:border-white/5 rounded-xl p-6.5 space-y-6 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
        ...
        {[
          ...
          <div key={idx} className="flex flex-col items-center text-center relative z-10 space-y-3 group">
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${item.color} text-slate-900 dark:text-white flex items-center justify-center shadow-lg group-hover:scale-[1.01] transition-transform`}>
              <item.icon size={24} />
            </div>
