...
const GuidedDiagnostics = ({ show, onClose, vehicle, onRequestCreated }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-2xl bg-slate-900 border-2 border-white/10 rounded-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <button 
          onClick={onClose}
          className="absolute top-5 right-5 z-10 w-8 h-8 rounded-full bg-black/40 hover:bg-black/60 text-slate-400 hover:text-white flex items-center justify-center border border-white/5 active-scale transition-all"
        >
          <Icons.X size={16} />
        </button>
                  ...
                  {symptoms.map((symptom) => {
                    ...
                    return (
                      <button
                        ...
                        className="p-4 rounded-xl bg-white/5 border border-white/5 hover:border-amber-500/30 transition-all flex items-center justify-between gap-4 text-left active-scale group"
                      >
                        <div className="flex items-center gap-4">
                          <div className="p-3 rounded-xl bg-slate-950 border border-white/10 text-slate-400 group-hover:text-amber-400 transition-colors">
                            <SymptomIcon size={20} />
                          </div>
                          <div>
                            <h4 className="text-sm font-black text-white uppercase tracking-tight">{symptom.label}</h4>
                            <p className="text-xs text-slate-400 mt-0.5 leading-snug">{symptom.desc}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
              ...
              >
                {isRecording && (
                  <>
                    <div className="text-center">
                      ...
                      <h3 className="text-3xl font-mono font-black text-white mt-4">
                        00:0{recordingTime}
                      </h3>
                    ...
                    {/* Canvas Waveform */}
                    <div className="w-full h-32 bg-slate-950 border border-white/5 rounded-xl relative overflow-hidden flex items-center justify-center shadow-inner">
                      <canvas
                        ref={canvasRef}
                        width={450}
                        height={128}
                    ...
                    <div className="w-full bg-slate-950/60 p-4 rounded-xl border border-white/5 space-y-1.5 font-mono text-[9px] text-slate-400">
                      ...
                      {telemetrySignal.length > 0 ? (
                        telemetrySignal.map((sig, idx) => (
                          <div key={idx} className="flex justify-between border-b border-white/5 pb-1">
                            ...
                            <span className="text-white font-black">{sig.split(":")[1]}</span>
                          </div>
                        ))
                      ) : (
                ...
                {isCompiling && (
                  <div className="text-center space-y-4 py-8">
                    ...
                    <h3 className="text-lg font-black text-white uppercase tracking-tight">AI TEŞHİS RAPORU DERLENİYOR</h3>
              ...
              >
                ...
                <div className="p-5 rounded-xl bg-amber-500/5 border border-amber-500/20 flex gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-black text-white uppercase mt-1.5 tracking-tight">
                ...
                <div className="bg-slate-950/60 rounded-xl border border-white/5 overflow-hidden">
                  <div className="px-4 py-3 bg-white/5 border-b border-white/5 flex justify-between items-center">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">TEKNİK ANALİZ RAPORU</span>
                  ...
                  <div className="p-4 space-y-3">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-400 font-bold">Ön Teşhis Hasar Tipi</span>
                      <span className="text-white font-black uppercase tracking-tight">{selectedSymptom.label}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-400 font-bold">Risk Derecesi</span>
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-400 font-bold">Tahmini Servis Süresi</span>
                      <span className="text-white font-black">{selectedSymptom.repairTime}</span>
                    </div>
                    ...
                    <div className="border-t border-white/5 pt-3 mt-2 flex justify-between items-center">
                      ...
                      <div className="text-right">
                        ...
                        <span className="text-[9px] text-slate-400 font-bold block mt-1">Carvis Marketplace Güvenceli</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button 
                    onClick={() => setStep(1)}
                    className="p-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all active-scale"
                  >
                    YENİDEN TEST ET
                  </button>
                  <button 
