    ...
    <div className="glass-card bg-slate-50 dark:bg-slate-950/40 p-8 rounded-[2.5rem] border border-black/10 dark:border-white/10 h-full relative overflow-hidden group lg:col-span-2 shadow-2xl backdrop-blur-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
        {prices.slice(0, 3).map((item, index) => (
          <div
            key={index}
            className="bg-white dark:bg-slate-900/60 border border-black/5 dark:border-white/5 p-5 rounded-2xl flex flex-col justify-between group/item hover:border-orange-500/30 transition-all"
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 font-black text-xs group-hover/item:text-slate-900 dark:text-white group-hover/item:bg-orange-600 transition-all">
                {item.name.substring(0, 1).toUpperCase()}
              </div>
              <div>
                <p className="text-sm font-black text-slate-900 dark:text-white font-sans uppercase tracking-tight">
                  {item.name}
                </p>
      ...
      <div className="mt-8 pt-6 border-t border-black/10 dark:border-white/10">
        <p className="text-[10px] text-slate-500 dark:text-slate-400 text-center font-sans font-bold uppercase tracking-widest opacity-80">
