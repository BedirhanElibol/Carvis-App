...
export const QuoteProvider = ({ children }) => {
    ...
    const fetchQuotes = async () => {
        ...
        // Guest users cannot query real DB (UUID mismatch)
        // Return empty or mock data if needed
        ...
        `)
            ...
            if (error) {
                // Sütun hatası varsa (örneğin migration henüz çalışmadıysa) basit sorguya düş
                if (error.code === '42703') {
                    console.warn("Retrying quote fetch with simplified fields due to schema mismatch");
                    const { data: fallbackData } = await supabase
                        .from('quotes')
                        .select(`*, customer:profiles!quotes_customer_id_fkey(full_name), seller:profiles!quotes_seller_id_fkey(full_name)`)
                        .or(`customer_id.eq.${currentUser.id},seller_id.eq.${currentUser.id}`)
                        .order('created_at', { ascending: false });
            ...
            showAlert('Sistem Hatası', `Teklifler yüklenirken hata oluştu: ${error.message || error.details || 'Bilinmeyen hata'}`, 'error');
        } finally {
            setLoading(false);
        }
    };
