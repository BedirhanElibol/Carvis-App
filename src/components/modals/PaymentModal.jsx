...
const PaymentModal = ({ show, onClose, amount, onSuccess, title = "Ödeme Yap" }) => {
            ...
            <div className="bg-slate-900 w-full max-w-md rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden relative">
                ...
                {step === 'card' && (
                    <div className="p-8">
                        ...
                        <form onSubmit={handlePayment} className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase ml-1 block mb-1">Kart Sahibi</label>
                                <input 
                                    required
                                    className="w-full bg-slate-950 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-primary-500 transition-colors uppercase"
                            ...
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase ml-1 block mb-1">Kart Numarası</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase ml-1 block mb-1">SKT</label>
                                    <input 
                                        required
                                        maxLength={5}
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-500 uppercase ml-1 block mb-1">CVV</label>
                                    <input 
                                        required
                                        type="password"
                                        maxLength={3}
                            ...
                            <div className="pt-4">
                                <button type="submit" className="w-full bg-primary-600 hover:bg-primary-500 text-white font-bold py-4 rounded-xl shadow-xl shadow-primary-900/50 active-scale flex items-center justify-center gap-2 transition-all">
                ...
                {step === 'success' && (
                    <div className="p-12 flex flex-col items-center text-center">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">Ödeme Başarılı!</h3>
                        <p className="text-slate-400 text-sm">İşleminiz onaylandı. Teşekkür ederiz.</p>
                    </div>
                )}
            </div>
        </div>
