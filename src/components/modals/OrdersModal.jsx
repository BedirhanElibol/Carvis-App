    ...
    <div className="fixed inset-0 bg-black/60 z-[85] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          {orders.length === 0 ? (
            <p className="text-center text-slate-500 py-10 font-sans">{t.noOrders}</p>
          ) : (
            orders.map((order) => (
              <div
                key={order.id}
                className="border border-slate-100 p-4 rounded-2xl bg-white shadow-md hover:shadow-lg transition"
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Badge
                    type={
                      order.status === "Teslim Edildi" ? "success" : "warning"
                    }
                  >
                    {order.status}
                  </Badge>
                ...
                <div className="flex justify-between items-end">
                  <p className="text-xs text-slate-500 font-sans">{order.date}</p>
                  <span className="font-bold text-orange-600 font-sans">
                    {order.price} ₺
                  </span>
                </div>
              </div>
