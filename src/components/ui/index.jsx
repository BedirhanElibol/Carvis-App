...
export const StatusBadge = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 font-black uppercase tracking-widest rounded-full ${config.bg} ${config.color} ${sizes[size]} ${className}`}
    >
      {" "}
      {showIcon && (
...
export const UrgencyBadge = ({
  ...
  return (
    <span
      className={`inline-flex items-center font-black uppercase rounded-full ${config.bg} ${config.text} ${sizes[size]} ${className}`}
    >
      {" "}
      {config.label}{" "}
    </span>
...
export const ItemTypeBadge = ({ type = "part", className = "" }) => {
  ...
  return (
    <span
      className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase ${config.bg} ${config.color} ${className}`}
    >
      {" "}
      {config.label}{" "}
    </span>
...
export const LoadingSpinner = ({
  ...
  return (
    <div
      className={`animate-spin rounded-full border-b-transparent ${sizes[size]} ${colors[color]} ${className}`}
    />
  );
}; // ================================================
